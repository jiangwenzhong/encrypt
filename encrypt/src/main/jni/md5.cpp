...
int cmpMD5(const char *key, const char *md5Str) {
    const char *SALT = "4f2e8a78c6a1250cfc4ddc5534540cb0";
    char *keyWithSalt = join(key, SALT);
    unsigned char digest[16], digestHex[16];
    MD5_CTX md5;
    MD5Init(&md5);
    MD5UpdaterString(&md5, keyWithSalt);
    MD5Final(digest, &md5);
    ByteToHexStr(digest, digestHex, 16);
    if (strcmp((const char *) digestHex, md5Str) == 0) {
        return 1;
    } else {
        return 0;
    }
